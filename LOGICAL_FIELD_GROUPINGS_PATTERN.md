...
## Overview
Through 2 hours of intensive work on the Assurance table, we've identified 5 distinct logical field groupings that have specific dependencies and interaction patterns.

## The 5 Logical Groupings
...
### **5. Policy-Level Boolean Flags**
- **Fields**: Buy/Sell, Key Man, Excluded Estate Duty, Excluded Provisions
- **Pattern**: Policy-wide checkboxes using rowSpan  
- **Behavior**: Boolean flags that apply to entire policy, displayed only on first row

## Key Architectural Insights

### Related Groups
- **Groups 2 & 3**: Share beneficiary row context and must synchronize when beneficiaries are added/removed
- **Groups 1, 4, 5**: Independent groups with distinct data flows

### Array Synchronization Requirements
- When beneficiaries change, Groups 2 & 3 arrays must expand/contract together
- Groups 4 & 5 remain unchanged during beneficiary operations

### Table Structure Dependencies
- Totals row must account for all 5 groups to maintain column alignment
- rowSpan usage applies only to Groups 4 & 5
...
## Future Applications
This grouping pattern can be applied to other calculator tables to:
