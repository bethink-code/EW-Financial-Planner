...
import { Chip, ChipRow, SectionHeading, StatusCard } from "./primitives";
...
export function TabRiskLt({ openPanel, managedFilter }: TabRiskLtProps) {
  ...
  const renderRow = (row: CoverRow) => {
    ...
    return (
      ...
      >
        ...
        <td className={cn(tdClass, "text-[13px] text-gray-500")}>
          {row.meta2}
        </td>
        <td className={cn(tdClass, "text-right font-semibold tabular-nums text-neutral-900")}>
          {row.premium}
        </td>
        <td className={tdClass}>
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            {tags.length > 0 && (
              <div className="mt-2">
                <div className="mb-1 text-[11px] text-gray-400">Benefits</div>
                <ChipRow tags={tags} />
              </div>
            )}
            {goals.length > 0 && (
    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-gray-400">3 policies</div>
        </div>
