...
import { ChipRow, SectionHeading, StatusCard } from "./primitives";
...
export function TabRiskSt({ openPanel, managedFilter }: TabRiskStProps) {
  ...
  const rows = SHORT_TERM_ROWS.filter((r) => {
  ...
  const renderRow = (row: CoverRow) => {
    ...
    return (
      ...
      >
        ...
        <td className={cn(tdClass, "text-[13px] text-gray-500")}>{row.meta2}</td>
    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-gray-400">1 policy</div>
        </div>
