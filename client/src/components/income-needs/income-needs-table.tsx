...
function IncomeNeedsTable({
  onAddIncomeNeed,
  showSummary = true,
}: IncomeNeedsTableProps) {
  const [location] = useLocation();
  const isRetirementNeed = location.startsWith("/needs/retirement");
  const [selectedIncomeNeedId, setSelectedIncomeNeedId] = React.useState<
    number | null
  >(null);
  ...
  const needIndex = (n: IncomeNeeds) =>
    incomeNeeds.findIndex((x) => x.id === n.id);
  ...
  const summaryCards = (
    <HybridSidebar
      ...
      renderActive={(n) => {
        ...
        if (n.termYears?.trim() && n.termYears !== "0")
          lines.push(`Term: ${n.termYears}`);
        return {
          subtitle: lines.join("\n") || "No details entered",
  ...
  // reference. No Entity column (the screenshot for income-needs doesn't show
  // one — these are household-level needs). DEL-specific groups hidden.
  const retirementDetailForm = selectedIncomeNeed
    ? (() => {
        const proj = projection?.incomeRequired.find(
          (p) => p.id === selectedIncomeNeed.id
        );
        const termNum = parseInt(selectedIncomeNeed.termYears) || 0;
        const nameValue = selectedIncomeNeed.name || "Income need";
        return (
          <GroupedDetailForm>
            <FieldGroup title="Regular income required (current value)">
              <div className="flex gap-3 flex-wrap items-end">
                ...
                <FormField label="Start">
                  <input
                    type="number"
                    defaultValue={parseInt(selectedIncomeNeed.startDate) || 0}
                    className="table-input"
                    style={{ width: "80px" }}
                    onBlur={(e) =>
                      handleTextBlur(
                ...
                <FormField label="Increase %">
                  <input
                    ...
                    defaultValue={formatPercentageValue(
                      selectedIncomeNeed.increasePercentage
                    )}
                    className={`table-input ${getValueClass(
                      selectedIncomeNeed.increasePercentage || "0%",
                      "percentage"
                    )}`}
                    style={{ width: "90px" }}
                    onFocus={handleDefaultValueFocus}
                ...
                <FormField label="Frequency">
                  <Select
                    value={selectedIncomeNeed.frequency || "monthly"}
                    onValueChange={(v) =>
      ...
      <div key={selectedIncomeNeed.id} className="space-y-10">
        <FieldGroup title="Overview">
          <div className="flex gap-3 flex-wrap">
            <FormField label="Name">
              <input
                ...
                className={`table-input ${getValueClass(
                  selectedIncomeNeed.name,
                  "text"
                )}`}
                style={{ width: "100%", minWidth: "200px" }}
                onBlur={(e) =>
                  handleTextBlur(selectedIncomeNeed.id, "name", e.target.value)
                }
              />
            </FormField>
            <FormField label="Description">
              <input
                ...
                className={`table-input ${getValueClass(
                  selectedIncomeNeed.description,
                  "text"
                )}`}
          ...
          <FormField label="Entity">
            <select
              className={`table-input table-dropdown ${getValueClass(
                selectedIncomeNeed.personName,
                "text"
              )}`}
  ...
  const aggregateRequired = projection?.incomeRequired ?? [];
  const totalCapital = aggregateRequired.reduce(
    (sum, n) => sum + (n.capitalAtRetirement ?? 0),
    0
  );
  const totalInCurrentTerms = aggregateRequired.reduce(
    ...
    <HybridViewWrapper
      ...
      header={
        <HybridHeaderBar
          add={
            onAddIncomeNeed
              ? { label: "Add Need", onClick: onAddIncomeNeed }
              : undefined
          }
          ...
          onDuplicate={
            selectedIncomeNeed
              ? () => handleDuplicate(selectedIncomeNeed)
              : undefined
          }
          onDelete={
            selectedIncomeNeed
              ? () => handleDelete(selectedIncomeNeed.id)
              : undefined
          }
          disabled={isUpdating}
        />
