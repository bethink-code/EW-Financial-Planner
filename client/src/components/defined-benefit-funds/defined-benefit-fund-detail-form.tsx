...
import { DefinedBenefitFund, ClientDetails } from "@shared/schema";
...
export function DefinedBenefitFundDetailForm({
  fund,
}: DefinedBenefitFundDetailFormProps) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      updates: Partial<DefinedBenefitFund>;
    }) => {
  ...
  const handleUpdate = useCallback(
    (field: keyof DefinedBenefitFund, value: string | string[] | boolean) => {
      setIsUpdating(true);
  ...
  const handleInputBlur = useCallback(
    (field: keyof DefinedBenefitFund, value: string) => {
  ...
  const handleOwnerChange = useCallback(
    (policyId: number, ownerIndex: number, newOwner: string) => {
  ...
  const handleOwnershipPercentageChange = useCallback(
    (policyId: number, ownerIndex: number, newPercentage: string) => {
  ...
  const handleRemoveOwner = useCallback(
    (policyId: number, ownerIndex: number) => {
  ...
  // Retirement-need view: minimal single-row form matching the Retirement
  // Funds pattern. DEL-specific groups (multi-owner table, death lump sum,
  // pension-at-death sub-section) are hidden — same component, route-driven.
        ...
        <FieldGroup title="Defined benefit fund">
          <div className="flex gap-3 flex-wrap items-end">
            ...
            <FormField label="Monthly Income">
              <input
                type="text"
                defaultValue={fund.finalMonthlySalary || "R 0"}
                className={`table-input ${getValueClass(
                  fund.finalMonthlySalary || "R 0",
                  "currency"
                )}`}
                style={{ width: "120px" }}
                onFocus={handleDefaultValueFocus}
                onBlur={(e) =>
                  handleInputBlur("finalMonthlySalary", e.target.value)
                }
                disabled={isUpdating}
              />
            </FormField>
            ...
            <FormField label="Increase %">
              <input
                ...
                className={`table-input ${getValueClass(
                  fund.pensionIncomeIncrease || "0%",
                  "percentage"
                )}`}
                style={{ width: "90px" }}
                onFocus={handleDefaultValueFocus}
                onBlur={(e) =>
                  handleInputBlur("pensionIncomeIncrease", e.target.value)
                }
                disabled={isUpdating}
              />
            </FormField>
            ...
            <FormField label="Lump sum (current terms)">
              <input
                type="text"
                defaultValue={fund.additionalTaxFreeAmount || "R 0"}
                className={`table-input ${getValueClass(
                  fund.additionalTaxFreeAmount || "R 0",
                  "currency"
                )}`}
                style={{ width: "140px" }}
                onFocus={handleDefaultValueFocus}
                onBlur={(e) =>
                  handleInputBlur("additionalTaxFreeAmount", e.target.value)
                }
                disabled={isUpdating}
              />
            </FormField>
    ...
    <GroupedDetailForm>
        ...
        <div className="space-y-4">
          ...
          <FormField label="Owners & Ownership Percentages">
            ...
            >
              ...
              <tbody>
                {Array.from(
                  { length: Math.max(fund.owners?.length || 1, 1) },
                  (_, rowIndex) => (
        ...
        >
          <FormField label="Years of Service">
            <input
              ...
              placeholder="0 years"
              className={`table-input ${
                fund.yearsOfService ? "" : "text-neutral-400"
              }`}
          ...
          <FormField label="Final Monthly Salary">
            <input
              ...
              placeholder="R 0"
              className={`table-input ${
                fund.finalMonthlySalary ? "" : "text-neutral-400"
              }`}
              style={{ width: "fit-content", minWidth: "120px" }}
              onFocus={handleDefaultValueFocus}
              onBlur={(e) =>
          ...
          <FormField label="Death Lump Sum">
            <input
              ...
              placeholder="R 0"
              className={`table-input ${
                fund.deathLumpSum ? "" : "text-neutral-400"
              }`}
              style={{ width: "fit-content", minWidth: "120px" }}
              onFocus={handleDefaultValueFocus}
          ...
          <FormField label="Additional Tax Free Amount">
            <input
              ...
              placeholder="R 0"
              className={`table-input ${
                fund.additionalTaxFreeAmount ? "" : "text-neutral-400"
              }`}
              style={{ width: "fit-content", minWidth: "120px" }}
              onFocus={handleDefaultValueFocus}
              onBlur={(e) =>
          ...
          >
            <FormField label="Amount">
              <input
                ...
                placeholder="R 0"
                className={`table-input ${
                  fund.pensionIncomeAmount ? "" : "text-neutral-400"
                }`}
                style={{ width: "fit-content", minWidth: "120px" }}
                onFocus={handleDefaultValueFocus}
                onBlur={(e) =>
                  handleInputBlur("pensionIncomeAmount", e.target.value)
                }
                disabled={isUpdating}
              />
            </FormField>
            ...
            <FormField label="Increase">
              <input
                ...
                placeholder="0%"
                className={`table-input ${
                  fund.pensionIncomeIncrease ? "" : "text-neutral-400"
                }`}
                style={{ width: "fit-content", minWidth: "120px" }}
                onFocus={handleDefaultValueFocus}
                onBlur={(e) =>
            ...
            {fund.pensionIncomeCheckbox && (
              <FormField label="Pension Income Years">
                <input
                  ...
                  placeholder="0 years"
                  className={`table-input ${
                    fund.pensionIncomeYears ? "" : "text-neutral-400"
                  }`}
                  style={{ width: "fit-content", minWidth: "120px" }}
                  onFocus={handleDefaultValueFocus}
                  onBlur={(e) =>
                    handleInputBlur("pensionIncomeYears", e.target.value)
                  }
                  disabled={isUpdating}
                />
              </FormField>
      ...
      {showRetirementProjection && (
        <FieldGroup title="Retirement Projection">
          <div
            className="grid grid-cols-3 gap-x-3 gap-y-4 items-end"
            style={{ width: "fit-content" }}
          >
            <FormField label="Growth Rate">
              <input
