...
export function RetirementFundTable({
  ...
  const fundIndex = (f: RetirementFund) =>
  ...
  const summaryCards = (
    <HybridSidebar
      ...
      renderActive={(f) => {
        ...
        if (f.fundValue && f.fundValue !== "R 0")
          lines.push(`Fund value: ${f.fundValue}`);
        return {
          subtitle: lines.join("\n") || "No details entered",
          primaryValue: f.fundValueAtDeath || "R 0",
          secondaryInfo:
            f.coverAmount && f.coverAmount !== "R 0"
              ? `Cover: ${f.coverAmount}`
              : undefined,
        };
      }}
  ...
  // Section summary: Retirement Funds feed into `Surplus capital` only
  // (not into Voluntary or Income surplus rows), so a standalone per-tab
  // total here is misleading. Hide the band on Retirement routes; DEL keeps
  // its existing aggregate summary when showSummary is on.
    ...
    <HybridViewWrapper
      ...
      header={
        <HybridHeaderBar
          ...
          onDuplicate={
            selectedFund ? () => onDuplicate(selectedFund) : undefined
          }
          onDelete={
