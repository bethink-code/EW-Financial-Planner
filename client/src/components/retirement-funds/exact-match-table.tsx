...
interface ExactMatchTableProps {
  ...
  onFieldUpdate: (id: number, field: keyof UpdateRetirementFund, value: string | string[]) => void;
...
export function ExactMatchTable({ 
  ...
  const handleCellUpdate = useCallback((id: number, field: keyof UpdateRetirementFund, value: string | string[]) => {
              ...
              <tr key={`${fund.id}-${type}-${ownerIndex}-${beneficiaryIndex}`} 
                ...
                <td className="border border-neutral-300 p-1">
                  {isMainRow && (
                    <input
                      ...
                      className={`${getFieldClass("text")} w-full text-xs`}
                ...
                <td className="border border-neutral-300 p-1">
                  {isMainRow && (
                    ...
                    >
                      <SelectTrigger className={`${getFieldClass("text")} w-full h-7 text-xs`}>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
