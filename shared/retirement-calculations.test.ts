...
const close = (a: number, b: number, tol = 1e-2) => expect(Math.abs(a - b)).toBeLessThan(tol);
...
describe("projectFundForward", () => {
  it("zero-year projection returns current value", () => {
    expect(
      projectFundForward({ currentValue: 1000000, monthlyContribution: 5000, contributionEscalation: 0.06, growthRate: 0.10, yearsToRetirement: 0 })
    ).toBe(1000000);
  });

  it("compounds a single sum at the growth rate", () => {
    const fv = projectFundForward({ currentValue: 100000, monthlyContribution: 0, contributionEscalation: 0, growthRate: 0.10, yearsToRetirement: 10 });
    close(fv, 100000 * Math.pow(1.10, 10), 0.5);
  });

  it("accumulates an escalating annuity", () => {
    const fv = projectFundForward({ currentValue: 0, monthlyContribution: 1000, contributionEscalation: 0.06, growthRate: 0.10, yearsToRetirement: 30 });
    expect(fv).toBeGreaterThan(0);
    // Sanity: should exceed flat contribution result.
    const flat = projectFundForward({ currentValue: 0, monthlyContribution: 1000, contributionEscalation: 0, growthRate: 0.10, yearsToRetirement: 30 });
    expect(fv).toBeGreaterThan(flat);
  });

  it("handles g == e (degenerate case)", () => {
    const fv = projectFundForward({ currentValue: 0, monthlyContribution: 1000, contributionEscalation: 0.10, growthRate: 0.10, yearsToRetirement: 10 });
    // Closed-form: C × n × (1+g)^(n-1) where C = annual = 12000, n = 10
    close(fv, 12000 * 10 * Math.pow(1.10, 9), 0.5);
  });
});

describe("presentValueGrowingAnnuity", () => {
  it("zero payment returns zero", () => {
    expect(presentValueGrowingAnnuity({ paymentPerPeriod: 0, escalation: 0.06, discount: 0.10, termYears: 25, periodsPerYear: 12 })).toBe(0);
  });

  it("zero term returns zero", () => {
    expect(presentValueGrowingAnnuity({ paymentPerPeriod: 1000, escalation: 0.06, discount: 0.10, termYears: 0, periodsPerYear: 12 })).toBe(0);
  });
  ...
  it("matches a known textbook PV (flat 1000/month, 8% discount, 10 years)", () => {
    ...
    const got = presentValueGrowingAnnuity({ paymentPerPeriod: 1000, escalation: 0, discount: 0.08, termYears: 10, periodsPerYear: 12 });
    close(got, expected, 1);
  });

  it("escalation = discount produces payment × periods", () => {
    const got = presentValueGrowingAnnuity({ paymentPerPeriod: 1000, escalation: 0.08, discount: 0.08, termYears: 10, periodsPerYear: 12 });
...
describe("dbPensionToCapitalEquivalent", () => {
  it("zero income returns zero", () => {
    expect(dbPensionToCapitalEquivalent({ monthlyIncome: 0, escalation: 0.06, realReturn: 0.04, yearsAfterRetirement: 25 })).toBe(0);
  });

  it("non-zero income produces positive capital", () => {
    const c = dbPensionToCapitalEquivalent({ monthlyIncome: 20000, escalation: 0.06, realReturn: 0.04, yearsAfterRetirement: 25 });
...
describe("requiredCapitalForIncome", () => {
  ...
  it("grosses up taxable income", () => {
    const net = requiredCapitalForIncome({ monthlyAmount: 50000, escalation: 0.06, termYears: 25, realReturn: 0.04, frequency: "monthly" });
...
describe("projectFutureInflow", () => {
  it("compounds value to inflow date", () => {
    const v = projectFutureInflow({ currentValue: 1000000, growthRate: 0.10, yearsAfterRetirement: 5, calculateCgt: false });
    close(v, 1000000 * Math.pow(1.10, 5), 0.5);
  });

  it("applies CGT haircut on gain when enabled", () => {
    const noCgt = projectFutureInflow({ currentValue: 1000000, growthRate: 0.10, yearsAfterRetirement: 5, calculateCgt: false });
    const withCgt = projectFutureInflow({ currentValue: 1000000, growthRate: 0.10, yearsAfterRetirement: 5, calculateCgt: true });
...
describe("additionalMonthlyContribution", () => {
  it("zero shortfall returns zero", () => {
    expect(additionalMonthlyContribution({ shortfallAtRetirement: 0, growthRate: 0.10, contributionEscalation: 0.06, yearsToRetirement: 20 })).toBe(0);
  });

  it("positive shortfall returns positive contribution that closes the gap", () => {
    const shortfall = 5_000_000;
    const years = 20;
    const monthly = additionalMonthlyContribution({ shortfallAtRetirement: shortfall, growthRate: 0.10, contributionEscalation: 0.06, yearsToRetirement: years });
    expect(monthly).toBeGreaterThan(0);

    // Round-trip: that monthly contribution, projected forward, should equal the shortfall.
    const fv = projectFundForward({ currentValue: 0, monthlyContribution: monthly, contributionEscalation: 0.06, growthRate: 0.10, yearsToRetirement: years });
...
describe("computeRetirementProjection (aggregator)", () => {
  const baseInput = {
    parameters: { id: 1, planId: 1, retirementAge: 65, retirementPlanningAge: 90, autoCalculateTax: true, lastUpdated: "" } as any,
    clientAge: 40,
    retirementFunds: [],
    definedBenefitFunds: [],
  ...
  it("income needs flow into capitalRequired and produce a shortfall + recommendation", () => {
    const result = computeRetirementProjection({
      ...baseInput,
      incomeNeeds: [{
        id: 1, description: "Living expenses", personName: "",
        startDate: "", termYears: "25", increasePercentage: "6%",
        cpi: false, frequency: "monthly", amount: "R 50,000", capitalisedAmount: "R 0",
      } as any],
    });
...
describe("isRetirementReadyToProject", () => {
  ...
  it("not ready without parameters", () => {
    expect(isRetirementReadyToProject({ ...base, parameters: null })).toBe(false);
  });
  ...
  it("not ready without capital source", () => {
    expect(isRetirementReadyToProject({
      ...base,
  ...
  it("ready with both", () => {
    expect(isRetirementReadyToProject({
      ...base,
      retirementFunds: [{ id: 1, fundValue: "R 1,000,000", monthlyContribution: "R 0" } as any],
      incomeNeeds: [{ id: 1, amount: "R 10,000" } as any],
    })).toBe(true);
  });
});
