...
- **Column Alignment Fix**: Implemented proper empty cell handling for rows without EntitySelector data to maintain table structure consistency
...
- **Implementation Template**: Assurance, Retirement Funds, Defined Benefit Funds, Income Needs, and Voluntary Investments serve as working examples of the complete pattern
- **Common Pitfalls Documentation**: Captured recurring implementation mistakes and solutions for future hybrid view rollouts
- **ADD BUTTON PATTERN**: Established new mandatory UX pattern for all hybrid views - AddButton positioned at top of tabs section with consistent styling and border separation. Applied across Income Needs, Defined Benefit Funds, Voluntary Investments, and Retirement Funds for improved user experience.
...
- **Global Hybrid View Pattern**: Battle-tested implementation framework with mandatory protocols in `GLOBAL_HYBRID_VIEW_PATTERN.md`. Provides 80% reusable infrastructure with strict border management rules (`isFirst`/`isLast` props required), no-spacing tab containers, clean detail forms, and 4 universal field groupings. Includes implementation checklist, visual testing protocol, and copy-paste templates. Successfully deployed across Assurance, Retirement Funds, Defined Benefit Funds, Income Needs, and Voluntary Investments with zero border issues. Preview cards use separate lines (`\n`) for multiple entity types (e.g., "Cover Beneficiaries: 2\nFund Beneficiaries: 1") for improved readability. Global CSS fixes prevent double borders at table bottoms. **Title Styling Standards**: Mandatory typography pattern (`text-lg font-semibold text-neutral-800`) ensures consistent individual item name display across all calculators, with header layout including title left and action buttons right. **Add Button Pattern**: NEW mandatory pattern places AddButton at top of tabs section with `hybrid-add-button-container p-4 border-b border-neutral-200` styling for consistent UX across all hybrid views.
...
- **EntitySelector Spacing Optimization**: Implemented horizontal spacing optimization using asymmetric padding (`pr-0.5 pl-1`, `px-0.5`, `pl-0.5 pr-1`) to create cohesive UI units while maintaining proper vertical alignment. Pattern documented in `ENTITY_SELECTOR_SPACING_PATTERN.md` for consistent application across all EntitySelector components.
- **Hybrid View Column Spacing Pattern**: Applied global spacing optimization to all side-by-side column layouts in hybrid view detail forms. Changed from `gap-x-6` (24px) to `gap-x-3` (12px) with `width: fit-content` constraint across Defined Benefit Funds, Voluntary Investments, Assets, and Liabilities components for compact, professional field grouping.
- **Preview Card Data Consistency Pattern**: Resolved owner/beneficiary count mismatches between table and hybrid views by implementing consistent empty string filtering across all preview card components. All components now use `item && item.trim() !== ''` filtering to accurately display only valid entries, eliminating confusion where preview cards showed fewer owners than table view. Pattern documented in `PREVIEW_CARD_DATA_CONSISTENCY_PATTERN.md`.
